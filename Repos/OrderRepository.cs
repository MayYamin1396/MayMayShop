    ...
    public class OrderRepository : IOrderRepository
    {
        ...
        public async Task<RemoveFromCartResponse> RemoveFromCart(RemoveFromCartRequest request, int userId,int platform)
        {
            ...
            else
            {
                ...
                //         UserId=userId,
                //         ActivityTypeId=MayMayShopConst.ACTIVITY_TYPE_ADD_TO_CART,
                //         Value=string.Format("removed {0} from cart.",productName),
                //         CreatedBy=userId,
                //         CreatedDate=DateTime.Now,
                //         PlatformId=platform,
                //     };
            ...
            #region  Product info
             ...
             var itemFromCart = await _context.TrnCart.Where(x => x.UserId == userId)
                .Select(s => new GetCartDetailProductInfo
                {
                    ...
                    Qty = s.Qty,
                    Name = _context.Product.Where(x => x.Id == s.ProductId).Select(s => s.Name)
                            .FirstOrDefault(),
                    Variation = null,
            ...
            using (var transaction = _context.Database.BeginTransaction())
            {
                ...
                await _memberPointRepo.ReceivedMemberPoint(recPMReq,token);
                    ...
                    if (req.PaymentInfo != null)
                    {
                        ...
                        //if payment is COD
                        if (req.PaymentInfo.PaymentServiceId == MayMayShopConst.PAYMENT_SERVICE_COD)
                        {
        ...
        public async Task<List<GetOrderHistoryResponse>> GetOrderHistory(GetOrderHistoryRequest request)
        {
            ...
            List<Order> orderList= await (  
                                            (   from ord in _context.Order
                                                ...
                                                && (request.PaymentStatusId==0 || ord.OrderPaymentInfo.Where(p=>p.OrderId==ord.Id && p.PaymentStatusId==request.PaymentStatusId).OrderByDescending(p=>p.TransactionDate).FirstOrDefault().PaymentStatusId==request.PaymentStatusId)
                    ...
                    if (orderDetailList.Count > 0)
                    {
                        ...
                        var paymentInfo = await _context.OrderPaymentInfo.Where(p => p.OrderId == orderDetails.Id)
                                    .Select(itm => new
                                    {
                                        ...
                                        PaymentServiceName = itm.PaymentService.Name,
                        ...
                        if (orderStatus != null)
                        {
                            orderHistory.OrderStatus = orderStatus.Name;
                        }
                        else
                        {
        ...
        public async Task<List<GetNotificationResponse>> GetNotificationBuyer(GetNotificationRequest request, int userId, string token)
        {
            ...
            if (notificationList.Count > 0)
            {
                foreach (var notificationDetails in notificationList)
                {
                    ...
                    notification.Body = notificationDetails.Body;
            ...
            if (productDetails.Count > 0)
            {
                ...
                foreach (var orderDetail in productDetails)
                {
                    ...
                    if (orderRes != null)
                    {
                        ...
                        {
                            userOrder.OrderStatus = orderStautsRes.Name;
                        }
        ...
        public async Task<ResponseStatus> UpdateOrderStatus(UpdateOrderStatusRequest request, int currentUserLogin)
        {
            ResponseStatus response = new ResponseStatus();
        ...
        public async Task<ResponseStatus> UpdatePaymentStatus(UpdatePaymentStatusRequest request, int currentUserLogin,string token)
        {
            ResponseStatus response = new ResponseStatus();
            ...
            {
                orderPaymentInfoDetail.SellerRemark=request.SellerRemark;
        ...
        public async Task<ResponseStatus> SellerOrderCancel(OrderCancelRequest request, int currentUserLogin)
        {
            ResponseStatus response = new ResponseStatus();
        ...
        public async Task<ResponseStatus> BuyerOrderCancel(OrderCancelRequest request, int currentUserLogin,string token)
        {
            ResponseStatus response = new ResponseStatus();
        ...
        public async Task<ResponseStatus> ChangeDeliveryAddress(ChangeDeliveryAddressRequest request)
        {
            ...
            if (order != null)
            {
                ...
                if (orderDelivery != null)
                {
                    ...
                    orderDelivery.Address = request.Address;
        ...
        public async Task<ResponseStatus> PaymentAgain(PaymentAgainRequest request, int currentUserLogin)
        {
            ResponseStatus response = new ResponseStatus();
        ...
        public async Task<ResponseStatus> PaymentApprove(PaymentApproveRequest request, int currentUserLogin)
        {
            ResponseStatus response = new ResponseStatus();
            var order = await _context.Order.Where(x => x.Id == request.OrderId && x.OrderUserId == currentUserLogin).FirstOrDefaultAsync();
            if (order != null)
        ...
        public async Task<GetOrderDetailResponse> GetOrderDetail(int orderId,string token)
        {
            ...
            resp.PaymentInfo = await _context.OrderPaymentInfo.Where(p => p.OrderId == orderId)
                .Select(itm => new GetOrderDeailOrderPaymentInfo
                {
                    ...
                    PaymentStatus = new GetOrderDetailPaymentStatus
                    {
                        ...
                        Name = itm.PaymentStatus.Name
                    },
                    PaymentService = new GetOrderDeailPaymentService
                    {
                        Name = itm.PaymentService.Name,
            ...
            if (resp.PaymentInfo.Count == 0)
            {
                ...
                paymentService1.Name = "အိမ်အရောက် ငွေရှင်းစနစ်ဖြင့် ဝယ်ယူထားပါသည်";
            ...
            resp.OrderItem = await _context.OrderDetail.Where(ordd => ordd.OrderId == orderId)
                .Select(itm => new GetOrderDetailOrderItem
                {
                    ...
                    Name = _context.Product.Where(prd => prd.Id == itm.ProductId).Select(x => x.Name).FirstOrDefault(),
                    Qty = itm.Qty,
        ...
        public async Task<PostOrderByKBZPayResponse> PostOrderByKBZPay(PostOrderRequest req, int userId, string token)
        {
            ...
            PostOrderByKBZPayResponse response = await _paymentservices.KBZPrecreate(transactionID,req.NetAmt,req.Platform);
